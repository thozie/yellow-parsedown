...
class YellowParsedownParser extends ParsedownExtra
{
	...
	{
		$text=preg_replace_callback("/<(\w+:[^\'\">\s]+)>/", array(
			&$this,
			"_doAutoLinks_url_callback"
		), $text);
		$text=preg_replace_callback("/<([\w\-\.]+@[\w\-\.]+)>/", array(
			&$this,
			"_doAutoLinks_email_callback"
		), $text);
		$text=preg_replace_callback("/\[(\w+)(.*?)\]/", array(
			&$this,
			"_doAutoLinks_shortcut_callback"
		), $text);
		$text=preg_replace_callback("/\[\-\-(.*?)\-\-\]/", array(
			&$this,
			"_doAutoLinks_comment_callback"
		), $text);
		$text=preg_replace_callback("/((http|https|ftp):\/\/\S+[^\'\"\,\.\;\:\s]+)/", array(
			&$this,
			"_doAutoLinks_url_callback"
		), $text);
		$text=preg_replace_callback("/([\w\-\.]+@[\w\-\.]+\.[\w]{2,4})/", array(
			&$this,
			"_doAutoLinks_email_callback"
		), $text);
		return $text;
	...
	function _doAutoLinks_shortcut_callback($matches)
	{
		...
		if(is_null($output))
			$output=htmlspecialchars($matches[0], ENT_NOQUOTES);
	...
	function _doHeaders_callback_setext($matches)
	{
		...
		if(empty($attr)&&$level>=2&&$level<=3)
			$attr=$this->getIdAttribute($text);
	...
	function _doImages_inline_callback($matches)
	{
		...
		if(!empty($alt))
			$output.=" alt=\"" . $this->encodeAttribute($alt) . "\"";
		if(!empty($title))
			$output.=" title=\"" . $this->encodeAttribute($title) . "\"";
		$output.=$attr;
...
$yellow->plugins->register("parsedown", "YellowParsedown", YellowParsedown::Version);
?>
